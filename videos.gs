...
function checkSheet() {
  ...
  var channelNames = indexSheet.getRange(1, 2, indexSheet.getLastRow() - 1).getDisplayValues();
  ...
  function checkVideoStatus(row) {
    ...
    var title = channelSheet.getRange(row, titleCol).getDisplayValue();
    var currentStatus = channelSheet.getRange(row, videoStatusCol).getDisplayValue();
    ...
    do {
      try {
        ...
        if (currentTime.getTime() - startTime.getTime() > 120000)
         break;
      }
    }
    while (responseText == null)
...
function checkRemovedVideos() {
  ...
  var siivaVideoTitles = siivaSheet.getRange(2, titleCol, siivaSheet.getLastRow() - 1).getDisplayValues();
  var siiva2VideoTitles = siiva2Sheet.getRange(2, titleCol, siiva2Sheet.getLastRow() - 1).getDisplayValues();
  var giivaVideoTitles = giivaSheet.getRange(2, titleCol, giivaSheet.getLastRow() - 1).getDisplayValues();
...
function checkPlaylistVideos() {
  ...
  var sheetVideoIds = channelSheet.getRange(2, idCol, channelSheet.getLastRow() - 1).getDisplayValues();
  var sheetWikiStatuses = channelSheet.getRange(2, wikiStatusCol, channelSheet.getLastRow() - 1).getDisplayValues();
...
function addRipsToSheet(videoIds, channel) {
  ...
  var logging = true; // Log the operations when true
...
function updateDatabase() {
  ...
  var videoIds = channelSheet.getRange(row, idCol, rowCount).getDisplayValues();
  var titles = channelSheet.getRange(row, titleCol, rowCount).getDisplayValues();
  var wikiStatuses = channelSheet.getRange(row, wikiStatusCol, rowCount).getDisplayValues();
  var videoStatuses = channelSheet.getRange(row, videoStatusCol, rowCount).getDisplayValues();
  var descriptions = channelSheet.getRange(row, videoDescriptionCol, rowCount).getDisplayValues();
  var viewCounts = channelSheet.getRange(row, videoViewsCol, rowCount).getDisplayValues();
  var likeCounts = channelSheet.getRange(row, videoLikesCol, rowCount).getDisplayValues();
  var dislikeCounts = channelSheet.getRange(row, videoDislikesCol, rowCount).getDisplayValues();
...
function checkDatabase()  {
  ...
  var channelSheet = spreadsheet.getSheetByName(channel);
  var videoIds = channelSheet.getRange("A2:A").getDisplayValues();
...
function addRipsToDatabase(idsToAdd, channel, channelDatabaseId) {
  ...
  for (var i in idsToAdd) {
    ...
    if (index != -1) {
      ...
      var title = channelSheet.getRange(row, titleCol).getDisplayValue();
      var wikiStatus = channelSheet.getRange(row, wikiStatusCol).getDisplayValue();
      var videoStatus = channelSheet.getRange(row, videoStatusCol).getDisplayValue();
      var uploadDate = channelSheet.getRange(row, videoUploadDateCol).getDisplayValue();
      var length = channelSheet.getRange(row, videoLengthCol).getDisplayValue();
      var description = channelSheet.getRange(row, videoDescriptionCol).getDisplayValue().replace(/NEWLINE/g, "\n");
      var viewCount = channelSheet.getRange(row, videoViewsCol).getDisplayValue();
      var likeCount = channelSheet.getRange(row, videoLikesCol).getDisplayValue();
      var dislikeCount = channelSheet.getRange(row, videoDislikesCol).getDisplayValue();
      var commentCount = channelSheet.getRange(row, videoCommentsCol).getDisplayValue();
